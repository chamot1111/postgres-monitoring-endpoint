...
do
  ...
  local method_overrides_redbean
  method_overrides_redbean = {
    connect = function(self, host, port, connect_opts)
        if type(host) == "string" then
            host = ParseIp(host)
        end
        return unix.connect(self.sock, host, port)
    end,
    receive = function(self, len)
        return unix.recv(self.sock, len)
    end,
    send = function(self, ...)
      return unix.send(self.sock, flatten(...))
    end,
    settimeout = function(self, t)
      return error("You attempted to call settimeout on a Redbean socket. This method is only available for the ngx cosocket API for releasing a socket back into the connection pool")
    end,
    setkeepalive = function(self)
  ...
  create_unixsocket = function(...)
    local proxy = {
      sock = unix.socket(...)
    }
    for k, v in pairs(method_overrides_redbean) do
      proxy[k] = v
    end
    -- return setmetatable(proxy, proxy_mt)
    return proxy
  ...
  new = function(socket_type)
    if socket_type == nil then
      if ngx and ngx.get_phase() ~= "init" then
        socket_type = "nginx"
      elseif type(GetRedbeanVersion) == "function" then
        socket_type = "redbean"
      else
